    ...
    try:
        for user in users:
            user_categories = (
                ...
                .filter(
                    UserCategory.is_income == False, UserCategory.user_id == user.id
                )
                .all()
            )
            ...
            for user_acc in user_accounts:
                for i in range(2):
                    ...
                    amount = Decimal(str(random.uniform(1, 10))).quantize(
                    ...
                    print(
                        f'Generated #{num_generated_transactions}, Transaction: {transaction.id}'
                    )
