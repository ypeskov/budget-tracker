...
async def analyze_spending_trends(
    request: Request, input_data: SpendingTrendsRequestSchema, analyzer: ExpenseAnalyzer = Depends(get_expense_analyzer)
) -> AnalysisResponseSchema:
    """
    ...
    try:
        ...
        if analysis is None:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Сервис анализа временно недоступен"
            )
...
async def get_budget_recommendations(
    ...
    """
    logger.info(
        f"Generating budget recommendations for user {request.state.user['id']}, "
        f"period: {input_data.start_date} to {input_data.end_date}"
    )

    try:
        recommendations = await analyzer.get_budget_recommendations(input_data.start_date, input_data.end_date)

        if recommendations is None:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Сервис рекомендаций временно недоступен"
            )
...
async def get_financial_insights(
    ...
    """
    logger.info(
        f"Generating financial insights for user {request.state.user['id']}, "
        f"period: {input_data.start_date} to {input_data.end_date}"
    )

    try:
        insights = await analyzer.get_financial_insights(input_data.start_date, input_data.end_date)

        if insights is None:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Сервис финансовых инсайтов временно недоступен"
            )
...
async def get_monthly_summary(
    request: Request, input_data: MonthlySummaryRequestSchema, analyzer: ExpenseAnalyzer = Depends(get_expense_analyzer)
) -> AnalysisResponseSchema:
    """
    Get AI-generated monthly financial summary.
    """
    logger.info(f"Generating monthly summary for user {request.state.user['id']}, month: {input_data.month_date}")
    ...
    try:
        ...
        if summary is None:
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Сервис месячных резюме временно недоступен"
            )
